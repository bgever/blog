<script lang="ts">
	import { SimpleLayout, Card, CardTitle, CardDescription, CardLink } from '$lib/ui';
	...
	const projects = [
		...
		},
		{
			name: 'Animaginary',
			description:
				'High performance web animations library that uses modern browser features.',
		...
		},
		{
			name: 'HelioStream',
			description:
				'Real-time video streaming library that optimizes for low latency playback.',
			link: { href: '#', label: 'github.com' },
		...
		},
		{
			name: 'Cosmos',
			description:
				'The operating system that powers our Planetaria space stations.',
			link: { href: '#', label: 'github.com' },
			logo: logoCosmos
		},
		{
			name: 'OpenShuttle',
			description:
				'The schematics for the first pointy, shuttle-shaped spacecraft.',
			link: { href: '#', label: 'github.com' },
...
<svelte:head>
	<title>Projects - Bart Verkoeijen</title>
	<meta
		name="description"
	...
	>
		{#each projects as project}
			<Card as="li">
				...
				>
					<img src={project.logo} alt="" class="h-8 w-8" />
				</div>
